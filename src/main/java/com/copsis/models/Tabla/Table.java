...
//          
            ...
            if (!header.isEmpty()) {
                ...
                // after a page break, we have to ensure that top borders get draw
...
//     
    ...
    private void drawVerticalLines2(Row<T> row) throws IOException {
    ...
    public float getHeaderAndDataHeight() {
        float height = 0;
        for (Row<T> row : rows) {
            //height += row.getHeight() + this.lineSpacing;
    ...
    public void addHeaderRow(Row<T> row) {
        //  this.header.add(row);//Este codigo fue modificado debido a que cuanto el contenido sobrepasa  a hoja ,crear otra fila en la siguiente hoja,pero que agrega la cabecera
        row.setHeaderRow(true);
    }

    public void addHeaderRowRestantes(Row<T> row) {
        this.header.add(row);
        row.setHeaderRow(true);
...
//    }
//
//    public List<Row<T>> deleteRows() {
//        rows = new ArrayList<>();
//        return rows;
//    }
//
//    public boolean setCallH(Boolean setBoole) {
//        this.cellCallH = setBoole;
//        return this.cellCallH;
//    }
