...
public class PDFTableStripper extends PDFTextStripper {
	 ...
	 */
	public  String getLer(String url) throws IOException
	{
        String razonSocial ="";
        ...
        final URL scalaByExampleUrl = new URL(url);
        document = PDDocument.load(scalaByExampleUrl.openStream());
        ...
        pdfStripper.setEndPage(1);
			...
			stripper.setRegion(new Rectangle((int) Math.round(1.0*res), (int) Math.round(1*res), (int) Math.round(250*res), (int) Math.round(250.0*res)));
				...
				for(int c=0; c<stripper.getColumns(); ++c) {
					...
					for(int r=0; r<stripper.getRows(); ++r) {
                        ...
                        if(stripper.getText(r, c).contains("Razón Social")){                       
                            razonSocial =stripper.getText(r, c).replace("Razón Social", "").replace("ción/:", "").replace("<\br>", "\n").replace("\n", " ").trim();
                        }
