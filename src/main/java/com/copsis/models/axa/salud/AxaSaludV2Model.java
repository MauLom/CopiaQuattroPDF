		...
		if (inicio > 0 && fin > 0 && inicio < fin) {
			...
			for (int i = 0; i < newcontenido.split("\n").length; i++) {
				...
				if (newcontenido.split("\n")[i].contains("plan") && newcontenido.split("\n")[i].contains(ConstantsValue.SOLICITUD_DOMICILIO)) {
					modelo.setPlan(newcontenido.split("\n")[i ].split(ConstantsValue.SOLICITUD_DOMICILIO)[1].split("###")[newcontenido.split("\n")[i ].split(ConstantsValue.SOLICITUD_DOMICILIO )[1].split("###").length -3] );
				...
				if (newcontenido.split("\n")[i].contains("C.P.") && newcontenido.split("\n")[i].contains("Fecha") &&  newcontenido.split("\n")[i].contains(ConstantsValue.VIGENCIA3)) {//C.P./fecha/vigencia
					...
					//vigencias
					modelo.setVigenciaDe(fn.formatDateMonthCadena(newcontenido.split("\n")[i].split(ConstantsValue.VIGENCIA3)[1].replace("###", "").replace("\r", "")));
                   cp=true;
                   vigenciaD = true;
				}	else if (newcontenido.split("\n")[i].contains("C.P.") && newcontenido.split("\n")[i].contains(ConstantsValue.VIGENCIA3)){
					modelo.setVigenciaDe(fn.formatDateMonthCadena(newcontenido.split("\n")[i].split(ConstantsValue.VIGENCIA3)[1].replace("###", "").trim().replace("\r", "")));
					vigenciaD = true;
				}
				...
				if (newcontenido.split("\n")[i].contains("Agente:")){
					...
					if(newcontenido.split("\n")[i+1].contains(ConstantsValue.PERIODO)) {
						modelo.setAgente((newcontenido.split("\n")[i].split(modelo.getCveAgente())[1].replace("###", " ") +" " +newcontenido.split("\n")[i+1].split(ConstantsValue.PERIODO)[0].trim()).trim().replace("@@@", "").replace("\r", "").replace("###", ""));
					}else {
						modelo.setAgente((newcontenido.split("\n")[i].split(modelo.getCveAgente())[1].split(ConstantsValue.PERIODO)[0].replace("###", " ") ).replace("\r", ""));
					}					
				}
		...
		if (inicio > 0 && fin > 0 && inicio < fin) {
			...
			for (int i = 0; i < newcontenido.split("\n").length; i++) {
				...
				if(newcontenido.split("\n")[i].contains(ConstantsValue.DEDUCIBLE)) {
					modelo.setDeducible(newcontenido.split("\n")[i].split(ConstantsValue.DEDUCIBLE)[1].replace("###", "").replace("\r", ""));
				}
