...
public class MapfreSaludModel {
	...
	// Variables
	private String inicontenido = "", contenido = "", newcontenido = "", fecha = "", resultado = "";;
	private int inicio = 0, fin = 0, index = 0, donde = 0, longitud_split = 0, longitud_texto = 0;
	    	...
	    	try {
	                ...
	                if (contenido.split("@@@").length >= 30) {
	                    ...
	                    if (fn.filtroPorRango(contenido, 30).indexOf("Contratante: ") > -1) {
	                        ...
	                        if (contenido.contains("CONTRATANTE")) {
	                            ...
	                            if (info.contains("DOMICILIO") && info.contains("R.F.C.:") && info.contains("ESTADO") && info.contains("POBLACIÓN")) {
	                                ...
	                                String cte_direccion = direccion.split("###")[1] + " " + direccion.split("ESTADO")[1].split("POBLACIÓN")[0];
	                                modelo.setCteDireccion(cte_direccion.replace("@@@", "").replace("###", "").replace("\r\n", ""));
	            ...
	            if (contenido.split("@@@")[4].contains("Domicilio: ")) {
	                ...
	                if (contenido.split("@@@")[4].trim().split("\r\n")[donde].trim().contains("Domicilio: ")
	                    ...
	                    modelo.setCteDireccion(contenido.split("@@@")[4].trim().split("\r\n")[donde].trim().substring(inicio, fin));
	                }
	            ...
	            if (donde > 0) {
	                ...
	                if (contenido.split("@@@")[donde].split("\r\n").length == 1) {
	                    if (contenido.split("@@@")[donde].split("\r\n")[0].split("###").length >= 2) {
	                        ...
	                        // vigencia_de
	                        modelo.setVigenciaDe(fn.formatDate(contenido.split("@@@")[donde].split("\r\n")[0].split("###")[1].trim(),"dd-MM-yy"));
	            ...
	            if (modelo.getVigenciaA().length() >= 0) {
	                if (contenido.contains("VIGENCIA###HASTA:")) {
	                    modelo.setVigenciaA(fn.formatDate(contenido.split("VIGENCIA###HASTA:")[1].split("CLIENTE")[0].replace("###", ""),"dd-MM-yy"));
	                } else if (contenido.contains("VIGENCIA###HASTA")) {
	                    modelo.setVigenciaA(fn.formatDate(contenido.split("VIGENCIA###HASTA")[1].split("DEL")[1].split("CLIENTE")[0].replace("###", "").replace(":", ""),"dd-MM-yy"));
	                }

	            }
	            if (modelo.getVigenciaDe().length() >= 0) {
	                if (contenido.contains("VIGENCIA###DESDE###LAS")) {
	                    modelo.setVigenciaDe(fn.formatDate(contenido.split("VIGENCIA###DESDE###LAS")[1].split("DEL")[1].split("TIPO")[0].replace("###", "").replace(":", ""),"dd-MM-yy"));
	                }
	            ...
	            if (donde > 0) {
	                ...
	                if (contenido.split("@@@")[donde + 1].split("\r\n").length == 1) {
	                    if (contenido.split("@@@")[donde + 1].split("\r\n")[0].split("###").length == 5) {
	                        ...
	                        modelo.setFormaPago(fn.formaPago(contenido.split("@@@")[donde + 1].split("\r\n")[0].split("###")[2].trim()));

	                        modelo.setMoneda(fn.moneda(contenido.split("@@@")[donde + 1].split("\r\n")[0].split("###")[3].replace("$", "").trim()));
	                    }
	                }
	            } else {
	                ...
	                if (donde > 0) {
	                    ...
	                    modelo.setFormaPago(fn.formaPago(contenido.split("@@@")[donde + 1].trim().split("\r\n")[0].trim().split("###")[1].trim()));
	                    modelo.setMoneda(fn.moneda(contenido.split("@@@")[donde + 1].split("\r\n")[0].split("###")[2].replace("$", "").trim()));
	                } else {
	            ...
	            if (modelo.getMoneda() >= 0) {
	                if (contenido.contains("MONEDA")) {
	                    ...
	                    modelo.setMoneda(fn.moneda(contenido.split("MONEDA:")[1].split("###")[1].split("\r\n")[0].replace("$", "").trim()));
	                }
	            }
	            String primas = "";
	            if (contenido.contains("Total:")) {
	                ...
	                for (String m : primas.split("\n")) {
	                    ...
	                    if (tm == 7) {
	                        modelo.setPrimaneta((fn.castFloat((m.split("###")[0]))));
	                        modelo.setDerecho((fn.castFloat(m.split("###")[3])));
	                        modelo.setRecargo((fn.castFloat(m.split("###")[2])));
	                        modelo.setIva((fn.castFloat(m.split("###")[5])));
	                        modelo.setPrimaTotal((fn.castFloat(m.split("###")[6])));
	                ...
	                primas = contenido.split("CONCEPTOS###ECONÓMICOS")[1].split("PADECIMIENTOS EXCLUIDOS")[0].replace("@@@", "").trim();
	                for (String m : primas.split("\n")) {
	                    int tm = m.split("###").length;
	                 
	                    if (tm == 7) {
	                        if (m.contains("NETA")) {
	                            modelo.setPrimaneta((fn.castFloat((m.split("###")[6]))));
	                        }
	                        if (m.contains("I.V.A")) {
	                            modelo.setIva((fn.castFloat(m.split("###")[6].replace("%", ""))));
	                        }
	                        if (m.contains("TOTAL")) {
	                               
	                            modelo.setPrimaTotal((fn.castFloat(m.split("###")[6].replace("$", "").trim())));
	                            if(m.split("###")[3].replace("$", "").trim().contains("|")){
	                            String valor = m.split("###")[3].replace("$", "").replace("|", "###");
	                            
	                            modelo.setRecargo((fn.castFloat(valor.split("###")[1].trim())).floatValue());
	                            }else{
	                            modelo.setRecargo((fn.castFloat(m.split("###")[3].replace("$", "").replace("0% |", "").trim())));
	                            }
	                    ...
	                    if (tm == 6) {
	                        if (m.contains("EXPEDICIÓN")) {
	                            modelo.setDerecho((fn.castFloat(m.split("###")[5])));
	                        }
	                    }
	                }
	            }
	            ...
	            if (inicontenido.indexOf("ANEXO DE BENEFICIARIOS:") > 0) {
	                ...
	                newcontenido = fn.eliminaSpacios(inicontenido.split("ANEXO DE BENEFICIARIOS:")[1].split("@@@ANEXO ")[0],' ',"doble").replace("######", "###") ;
	                ...
	                if (contenido.split("@@@")[17].split("\r\n")[0].contains("a###a###a###a###a###a###a")) {
	                        ...
	                        for (String x : datos.split("\r\n")) {
	                            ...
	                            switch (x.split("###").length) {
	                                ...
	                                case 5:
	                                    if (fn.isNumeric(x.split("###")[4].trim())) {
	                                        if (x.split("###")[4].trim().length() == 8) {
	                                            fecha = x.split("###")[4].substring(4, 8) + "-" + x.split("###")[4].substring(2, 4) + "-" +  x.split("###")[4].substring(0, 2);
	                                        }
	                                ...
	                                case 4:
	                                    ...
	                                    asegurado.setNombre(x.split("###")[0].replace("@@@", "").replace("38", "").replace("13", "").trim());
	                    ...
	                    if (conten.contains("%") || conten.contains("OBERTURAS")) {
	                        ...
	                        if (inicio > 0 && fin > 0 && inicio < fin) {
	                            ...
	                            if (newcontenido.split("\r\n")[1].contains("ASEGURADO:") && newcontenido.split("\r\n")[2].contains("SEXO:")) {
	                            	asegurado.setNombre(newcontenido.split("ASEGURADO:")[1].split("SEXO")[0].replace("###", " ").trim());
	                            }
	                            if (newcontenido.split("\r\n")[3].contains("NACIMIENTO:") && newcontenido.split("\r\n")[4].contains("EDAD:")) {
	                            	asegurado.setNacimiento(fn.formatDate(newcontenido.split("NACIMIENTO:")[1].split("EDAD")[0].replace("###", " ").replace("DE", "/").replace(" ", "").trim(),"dd-MM-yy"));
	                            }
	                            asegurados.add(asegurado);
                            	modelo.setAsegurados(asegurados);
	            ...
	            if (modelo.getAsegurados().size() == 0) {
	                ...
	                if (inicio > 0 && fin > 0 && inicio < fin) {
	                    ...
	                    String dato = inicontenido.substring(inicio, fin).replace("ASEGURADOS", "");
	                    
	                    System.out.println(dato);

	                    donde = dato.indexOf("NOMBRE###EDAD###SEXO###EXTRAPRIMA###ANTIGÜEDAD###ANTIGÜEDAD");

	       
	                    if (donde > 0) {
	                        newcontenido = dato.substring(donde, dato.indexOf("NETA"));
	                    } else {
	                    	newcontenido = dato;
	                    }
	                    newcontenido = fn.RemplazaGrupoSpace(newcontenido);
	                    for (String x : newcontenido.split("\n")) {
	                    	EstructuraAseguradosModel asegurado = new EstructuraAseguradosModel();
	                        ...
	                        if (x.contains("FECHA") && x.contains("Av.")) {
	                            ...
	                            if (nsp == 4) {
	                                ...
	                                asegurado.setNombre(x.split("###")[0].replace("@@@", "").substring(0, x.split("###")[0].replace("@@@", "").length() - 2));
	                                asegurado.setSexo(fn.sexo(x.split("###")[2]) ? 1:0);
	                                asegurados.add(asegurado);
	                            	modelo.setAsegurados(asegurados);
	                            }
...
//	            donde = fn.recorreContenido(contenido, "DEDUCIBLE  COAS         TOPE COAS ZONA HOSPITAL MÍNIMO"); // VALORES DE
//
//	            // NIVEL
//	            // RAIZ
//	            if (donde > 0) {
...
//	            }
//
//	            int inicoberturas = contenido.indexOf("MÍNIMO") + 8;
//	            int fincoberturas = contenido.indexOf("ANTERIORES") - 10;
//	            List<EstructuraCoberturasModel> coberturas = new ArrayList<>();
//	            if (inicoberturas > -1 && fincoberturas > -1 & inicoberturas < fincoberturas) {
//	            	
//	            	newcontenido =  contenido.substring(inicoberturas, fincoberturas);
//	               
//
...
//	                        	
//	                        	cobertura.setNombre(datod.split("###")[0].replace("\r\n", "").replace("@@@", ""));
//	                        	cobertura.setSa(datod.split("###")[1].replace("\r", "").replace("@@@", "") + " " + datod.split("###")[2].replace("\r\n", "").replace("@@@", ""));
...
//
//	                    if (x.contains("a###a###a###a###a###a") || x.contains("@@@COBERTURAS###SUMA###DEDUCIBLE") || x.contains("AMPARADAS###ASEGURADA###COASEGURO") || x.contains("(Continuación)")) {
//	                    } else {
//
//	                        switch (numgatos) {
//	                            case 2:
...
//	            if (inicio > 0 && fin > 0 && inicio < fin) {
//	                fecha_emisi = contenido.split(strb)[1].split("Prima Neta")[0].split("\n")[1].replace("@@@", "###").trim();
//	                modelo.setFechaEmision(fn.formatDate(fecha_emisi.split("###")[1].trim(),"dd-MM-yy"));
//
//	            } else {
//	                inicio = contenido.indexOf("EMISIÓN");
//	                if (inicio > 0) {
//	                    fecha_emisi = contenido.split("EMISIÓN")[1].split("\r\n")[0];
//	                    modelo.setFechaEmision(fn.formatDate(fecha_emisi.split("###")[1],"dd-MM-yy"));
//	                }
//
//	            }
//
...
//	           	modelo.setRecibos(recibos);
				...
				modelo.setError(MapfreSaludModel.this.getClass().getTypeName() +" | "+ ex.getMessage() + " | " + ex.getCause());
		         return modelo;
			}
	  }
}
