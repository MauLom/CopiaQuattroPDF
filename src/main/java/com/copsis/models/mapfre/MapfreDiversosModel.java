...
public class MapfreDiversosModel {
	...
	// Variables
	private String inicontenido = "", contenido = "", newcontenido = "",seccion="",ubicacionesTex ="",
			resultado = "",recibosText;
	private int inicio = 0, fin = 0, index = 0;
	...
	public EstructuraJsonModel procesar() {
		contenido = fn.remplazarMultiple(contenido, fn.remplazosGenerales());
		 ...
		 contenido = contenido.replace("Póliza Número    :", "Póliza Número:")
	                ...
	                .replace("Vig encia", "Vigencia")
	                .replace("Clave de", "###Clave de")
	                .replace("T e l . :", "Tel.:")
	                .replace(" C O P S E ,  A G E NTE", "COPSE AGENTE")
	                .replace("Cliente MAPFRE   :", "Cliente MAPFRE:")
	                .replace("Cliente Mapfre###:", "Cliente Mapfre:")
	                .replace("Cobro:", "cobro:")//Cobro:
		...
		try {
		     ...
		     // tipo
	            ...
	            if (inicio > -1) {
	                modelo.setEndoso(contenido.substring(inicio + 13, inicio + 100).split("\r\n")[0].replace(":", "").trim());
	            }
	            ...
	            if (inicio > -1) {
	                ...
	                if (newcontenido.contains("R.F.C.")) {
	                    modelo.setCteNombre(fn.gatos(contenido.substring(inicio + 12, inicio + 150).split("\r\n")[0].split("R.F.C.")[0].trim()));
	                }
	            } else {
	                inicio = contenido.indexOf("R.F.C.:");
	                if (inicio > -1) {
	                    modelo.setCteNombre(fn.gatos(contenido.substring(inicio + 7, inicio + 150).split("\r\n")[0].split("R.F.C.:")[0].trim()));
	                }
	            }
	                ...
	                switch (newcontenido.split("###").length) {
	                    ...
	                    case 3:
	                        if (newcontenido.split("###")[0].contains("de:")) {;
	                            modelo.setVigenciaDe(fn.formatDate(newcontenido.split("###")[0].split("de:")[1].trim(),"dd-MM-YY"));
	                        }
	                        break;
	                }
	            }
	            ...
	            if (inicio > -1 && fin > inicio) {
	                ...
	                if (newcontenido.contains("de:")) {
	                    ...
	                    switch (newcontenido.split("###").length) {
	                        case 3:
	                            modelo.setVigenciaA(fn.formatDate(newcontenido.split("###")[0].trim(),"dd-MM-YY"));
	            ...
	            if (inicio > -1 && fin > inicio) {
	                newcontenido = fn.RemplazaGrupoSpace(contenido.substring(inicio + 6, fin).replace("@@@", "").trim().split("\r\n")[0]).replace("######", "###");
	                switch (newcontenido.split("###").length) {
	                    case 5:
	            ...
	            if (inicio > -1) {
	                modelo.setIdCliente(fn.gatos(contenido.substring(inicio + 15, contenido.indexOf("\r\n", inicio)).trim()));
	            }
	            ...
	            if (inicio > -1 && fin > inicio) {
	                ...
	                if (newcontenido.contains("otal:")) {
	                    newcontenido = fn.RemplazaGrupoSpace(newcontenido.split("otal:")[1].replace("@@@", "").trim()).replace("######", "###");
	                    switch (newcontenido.split("###").length) {
	                        case 7:
	            ...
	            // renovacion
	            inicio = contenido.indexOf("SUMA ASEGURADA DEDUCIBLE");
	            fin = contenido.indexOf("ABREVIATURAS");
	            ...
	            if (inicio > -1 && fin > inicio) {
	                ...
	                for (String x : newcontenido.split("\r\n")) {
	                    if (x.contains("BIENES Y RIESGOS CUBIERTOS") == false && x.contains("---") == false
	                        ...
	                        resultado = resultado.replace("A.###MIN", "A. MIN").replace("###DSMG", " DSMG").replace(".###MIN", ". MIN").replace(".###MAX", ". MAX");
	                ...
	                inicio = contenido.indexOf("BIENES Y RIESGOS");
	                fin = contenido.indexOf("ABREVIATURAS");
	              
	                if (fin == -1) {
	                    fin = contenido.indexOf("RENOVACION AUTOMATICA");
	                }
	                ...
	                if (inicio > -1 && fin > inicio) {
	                    ...
	                    for (String x : newcontenido.split("\r\n")) {
	                        ...
	                        if (x.contains("SUMA ASEGURADA") == false && x.contains("---") == false) {
	                        	  System.out.println(inicio +"---------------------_> " + x);
	                        	  EstructuraCoberturasModel cobertura = new EstructuraCoberturasModel();
	                            x = fn.RemplazaGrupoSpace(x.trim());
	                            x = x.replace("###$###", "###$ ");
	            ...
	            if (inicio > -1 && fin > inicio) {
	                ...
	                if (inicio > -1) {
	                    newcontenido = contenido.substring(inicio, contenido.indexOf(".", inicio)).split(":")[1].replace("\r\n", "").replace("CONSTRUCCION", "CONSTRUCCIÓN").trim();
	                    ...
	                    if (fin > -1) {
	                        ...
	                        resultado = newcontenido.substring(fin + 3, newcontenido.length()).replace("TECHO DE", "").trim();
			...
			modelo.setError(
					MapfreDiversosModel.this.getClass().getTypeName() + " | " + ex.getMessage() + " | " + ex.getCause());
			return modelo;
		}
	        ...
	        try {
	            ...
	            if (inicio > -1) {
	                ...
	                if (newcontenido.contains("Hasta: 12:00 horas del")) {
	                    ...
	                    recibo.setVigenciaA(fn.formatDate(resultado, "dd-MM-yy") );
	                }
	            }
