		...
		try {
			...
			for (int i = 0; i < newcontenido.toString().split("\n").length; i++) {								
				if(newcontenido.toString().split("\n")[i].contains("RFC:") && newcontenido.toString().split("\n")[i].contains("Cliente:")) {
					modelo.setRfc(newcontenido.toString().split("\n")[i].split("RFC:")[1].split("Cliente:")[0].replace("###", "").trim() );
				}
				if(newcontenido.toString().split("\n")[i].contains("Nombre:")) {
