...
public class AigDiversosModel {
	...
	public EstructuraJsonModel procesar() {
		...
		try {
			...
			boolean cbo = false;
			for (int i = 0; i < contenido.split("SECCIONES Y COBERTURAS").length; i++) {
				if (contenido.split("SECCIONES Y COBERTURAS")[i].contains("MONEDA")) {
					newcoberturas = contenido.split("SECCIONES Y COBERTURAS")[i].split("MONEDA")[0];
				} else if (contenido.split("SECCIONES Y COBERTURAS")[i].contains("En cumplimiento") && cbo == false) {
					newcoberturas += contenido.split("SECCIONES Y COBERTURAS")[i].split("En cumplimiento")[0];
					cbo = true;
				}
			}
			...
			if (newcoberturas.length() > 0) {
				...
				newcoberturas = newcoberturas.replace("@@@", "").replace("\r", "").trim();
				for (int i = 0; i < newcoberturas.split("\n").length; i++) {
					EstructuraCoberturasModel cobertura = new EstructuraCoberturasModel();
					if (newcoberturas.split("\n")[i].split("###").length > 1) {
						cobertura.setNombre(newcoberturas.split("\n")[i].split("###")[0].trim());
						cobertura.setSa(newcoberturas.split("\n")[i].split("###")[1].trim());
