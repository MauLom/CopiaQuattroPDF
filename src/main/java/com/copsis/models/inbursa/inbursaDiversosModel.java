	        ...
	        modelo.setCia(35);
		      ...
		      inicio = contenido.indexOf("PÓLIZA DE SEGUROS");
	            ...
	            if(inicio > 0 &&  fin >  0 && inicio < fin) {
	            	newcontenido.append(contenido.substring(inicio, fin).replace("\r","").replace("@", "").replace("las 12:00 hrs. del", ""));        
	            	for (int i = 0; i < newcontenido.toString().split("\n").length; i++) {
						if(newcontenido.toString().split("\n")[i].contains("PÓLIZA") && newcontenido.toString().split("\n")[i].contains("CIS") && newcontenido.toString().split("\n")[i].contains("ID CLIENTE")) {
	            		...
	            		if(newcontenido.toString().split("\n")[i].contains("FINANCIAMIENTO") && newcontenido.toString().split("\n")[i].contains("R.F.C:") ) {
	               		...
	               		if(newcontenido.toString().split("\n")[i].contains("IVA") && newcontenido.toString().split("\n")[i].contains("FORMA DE PAGO") && iva  == false) {
	               			...
	               			if(newcontenido.toString().split("\n")[i+1].split("###").length > 4) 
	               			{
	               				if(newcontenido.toString().split("\n")[i+1].contains("PRIMA TOTAL") && newcontenido.toString().split("\n")[i+1].contains("Horas") ) {
	               		...
	               		if(newcontenido.toString().split("\n")[i].contains("PRIMA TOTAL") && newcontenido.toString().split("\n")[i].contains("Desde")) {
	               			if(newcontenido.toString().split("\n")[i].split("-").length > 2) {
	               				modelo.setPrimaTotal(fn.castBigDecimal(fn.castDouble(fn.extraerNumeros(newcontenido.toString().split("\n")[i].split("PRIMA TOTAL")[1].split("###")[1]).replace("###", ""))));
		               			if(modelo.getPrimaTotal().toString().length() > 0) {
		               				String x =newcontenido.toString().split("\n")[i].split(fn.extraerNumeros(newcontenido.toString().split("\n")[i].split("PRIMA TOTAL")[1].split("###")[1]).replace("###", ""))[1];
