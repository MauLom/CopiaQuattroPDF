	...
	public EstructuraJsonModel procesar() {
		   ...
		   contenido = fn.remplazarMultiple(contenido, fn.remplazosGenerales());
	        ...
	        try {
	            ...
	            if (inicio > 0) {
	                ...
	                for (int i = 0; i < newcontenido.split("\n").length; i++) {
	                    if (newcontenido.split("\n")[i].contains("/") & newcontenido.split("\n")[i].split("/").length > 2) {
	                        modelo.setFechaEmision(fn.formatDate(newcontenido.split("\n")[i].split("###")[2].trim(),"dd-MM-yy"));
	                    }
	                }
	            }
	                    ...
	                    if (contenido.split("@@@")[donde + 1].split("\r\n").length == 1) {
	                        if (contenido.split("@@@")[donde + 1].split("###").length == 5) {
	                            ...
	                            if (contenido.split("@@@")[donde + 1].split("###")[3].trim().equals("Neta")) {
	                                modelo.setPrimaneta(fn.castFloat(contenido.split("@@@")[donde + 1].split("###")[4].trim()));
	                            }
	                        }
	                    }
	                }
	                ...
	                if (contenido.split("@@@")[donde].split("\r\n").length == 1) {
	                    ...
	                    if (contenido.split("@@@")[donde + 1].split("\r\n").length == 1) {
	                        switch (contenido.split("@@@")[donde + 1].split("###").length) {
	                            case 6:
	                                modelo.setModelo(fn.castInteger(fn.remplazarMultiple(contenido.split("@@@")[donde + 1].split("###")[0].trim(), fn.remplazosGenerales())));
	                                ...
	                                if (contenido.split("@@@")[donde + 1].split("###")[4].trim().equals("Póliza")) {
	                                    modelo.setDerecho(  fn.castFloat(contenido.split("@@@")[donde + 1].split("###")[5].trim()));
	                            ...
	                            case 7:
	                                ...
	                                if (contenido.split("@@@")[donde + 1].split("###")[5].trim().equals("Póliza")) {
	                                    modelo.setDerecho(fn.castFloat(fn.remplazarMultiple(contenido.split("@@@")[donde + 1].split("###")[6].trim(), fn.remplazosGenerales())));	                                    
	                                }
	                                break;
	                        }
	                    }
	                ...
	                if (inicio > 0 & fin > 0 & inicio < fin) {
	                    ...
	                    for (int i = 0; i < newcontenido.split("\n").length; i++) {
	                        ...
	                        if (newcontenido.split("\n")[i].contains("Descripción")) {
	                            modelo.setPrimaneta(fn.castFloat(fn.remplazarMultiple(newcontenido.split("\n")[i].split("Neta")[1].replace("###", ""), fn.remplazosGenerales())));
	                            modelo.setDescripcion(newcontenido.split("\n")[i + 1].split("###")[0]);
	                            modelo.setSerie(newcontenido.split("\n")[i + 1].split("###")[1]);
	                        }
	                        if (newcontenido.split("\n")[i].contains("Modelo")) {
	                            modelo.setDerecho(fn.castFloat(fn.remplazarMultiple(newcontenido.split("\n")[i].split("Póliza")[1].replace("###", ""), fn.remplazosGenerales())));
	                            modelo.setModelo(fn.castInteger(fn.remplazarMultiple(newcontenido.split("\n")[i + 1].split("###")[0],fn.remplazosGenerales())));
	                            modelo.setMotor(newcontenido.split("\n")[i + 1].split("###")[1]);
	                        }
	                        
	                         if (newcontenido.split("\n")[i].contains("Importe")) {
	                               modelo.setPrimaTotal(fn.castFloat(fn.remplazarMultiple(newcontenido.split("\n")[i].split("Pagar")[1].replace("###", ""), fn.remplazosGenerales())));
	                ...
	                if (contenido.split("@@@")[donde].split("\r\n").length == 1) {
	                    if (contenido.split("@@@")[donde].split("###").length == 7) {
	                        if (contenido.split("@@@")[donde].split("###")[5].trim().equals("Pagar")) {
	                            ...
	                            modelo.setPrimaTotal(fn.castFloat(fn.remplazarMultiple(contenido.split("@@@")[donde].split("###")[6].trim(), fn.remplazosGenerales())));
	                        }
	                    }
	                }
	            }
	                    ...
	                    for (int i = 0; i < contenido.split("@@@")[donde].split("\r\n").length; i++) {
	                        if (i == 0) {
	                            if (contenido.split("@@@")[donde].split("\r\n")[i].contains("Forma###de###Pago###Moneda")) {
	                                if (contenido.split("@@@")[donde].split("\r\n")[i + 1].split("###").length == 4) {
	                                    if (contenido.split("@@@")[donde].split("\r\n")[i + 1].split("###")[1].trim().split(" ").length == 1
	                                            && contenido.split("@@@")[donde].split("\r\n")[i + 1].split("###")[2].trim().split(" ").length == 1) {
	                                        modelo.setFormaPago(fn.formaPago(contenido.split("@@@")[donde].split("\r\n")[i + 1].split("###")[1].trim()));
	            ...
	            //plan
	            if (contenido.split("@@@")[1].contains("No. Póliza") && contenido.split("@@@")[1].contains("Regular Autos")) {
	                inicio = -1;
	                fin = -1;
	            ...
	            if (donde > 0) {
	                if (contenido.split("@@@")[donde].split("\r\n").length == 2) {
	                    if (contenido.split("@@@")[donde].split("\r\n")[1].split("###").length == 4 && contenido.split("@@@")[donde].split("\r\n")[1].contains("Edad###Sexo") == false) {
	            ...
	            return modelo;
	        	 ...
	        	 modelo.setError(GnpAutosModel.this.getClass().getTypeName() +" | "+ ex.getMessage() + " | " + ex.getCause());
		            return modelo;
	        }
