...
public class ChubbDiversosModel {
		...
		try {
				...
				for (int i = 0; i < newcontenido.split("\n").length; i++) {
					...
					if (newcontenido.split("\n")[i].contains(ConstantsValue.DOMICILIO)
						...
						modelo.setCteDireccion((newcontenido.split("\n")[i].split(ConstantsValue.DOMICILIO)[1]
								.split(ConstantsValue.TELEFONOAC)[0]
								+ " " + newcontenido.split("\n")[i + 1].split("RFC:")[0]).replace("###", " ")
								.replace("\r", "").trim());
					} else if (newcontenido.split("\n")[i].trim().split(ConstantsValue.DOMICILIO).length > 1
						...
						direccion = fn.eliminaSpacios(
								direccion.replace("###", " ").replace("@@@", "").split("C.P:")[0].trim());
					...
					if (modelo.getCteNombre().length() == 0 && newcontenido.split("\n")[i].contains("propietario")
						...
						modelo.setCteNombre(
								fn.elimgatos(newcontenido.split("\n")[i + 1].split(ConstantsValue.ASEGURADO)[1].trim())
										.split("###")[0]);
					...
					if (newcontenido.split("\n")[i].contains("Moneda:")
						...
						modelo.setMoneda(fn.moneda(newcontenido.split("\n")[i].split("Moneda:")[1].split("Forma")[0]
								.replace("###", "").trim()));
						modelo.setFormaPago(
								fn.formaPagoSring(newcontenido.split("\n")[i].split("pago:")[1].replace("\r", "")
										.replace("###", "").trim()));
					}
					...
					if (modelo.getFechaEmision().length() == 0
							&& newcontenido.split("\n")[i].contains(ConstantsValue.EMISION2)
							&& newcontenido.split("\n")[i].trim().split(ConstantsValue.EMISION2).length > 1
							&& newcontenido.split("\n")[i].contains("Referencia")) {

						String x = newcontenido.split("\n")[i].split(ConstantsValue.EMISION2)[1].split("Referencia")[0]
								.trim().replace("P. M.", "P/M").replace("P.M", "P/M");
						if (x.contains("P/M")) {
			...
			if (inicio > -1 && fin > inicio) {
				...
				for (int i = 0; i < newcontenido.split("\n").length; i++) {
					...
					if (modelo.getRecargo() == null && modelo.getRecargo().intValue() == 0
					...
					if ((modelo.getDerecho() == null || modelo.getDerecho().intValue() == 0)
							&& newcontenido.split("\n")[i].contains(ConstantsValue.EXPEDICION)) {
						List<String> valores = fn.obtenerListNumeros2(newcontenido.split("\n")[i].replace(",", ""));
						if (!valores.isEmpty()) {
			...
			for (int i = 0; i < ubicacionesExt.split(ConstantsValue.CARACTERISTICASRIESGO).length; i++) {
				if (i > 0) {
					contenidoUb
							.append(ubicacionesExt.split(ConstantsValue.CARACTERISTICASRIESGO)[i].split("Coberturas")[0]
									+ "\n");
				}
			}

			if(contenidoUb.toString().contains("Giro")){
			...
			for (int i = 0; i < contenidoUb.toString().split("\n").length; i++) {
				...
				if (contenidoUb.toString().split("\n")[i].contains("Dirección")) {
					ubicacion.setCalle(contenidoUb.toString().split("\n")[i]
							.split(fn.palabraRgx(contenidoUb.toString().split("\n")[i], ConstantsValue.DIRECCION2))[1]
					...
					if (tlineas > 5 && contenidoUb.toString().split("\n")[i + 4].contains(ConstantsValue.TECHOS)) {
						ubicacion.setTechos(
								fn.material(contenidoUb.toString().split("\n")[i + 4].split(ConstantsValue.TECHOS)[1]
										.split(ConstantsValue.INCENDIOMYM)[1].toUpperCase().replace("###", "").trim()));
						ubicacion.setMuros(
								fn.material(contenidoUb.toString().split("\n")[i + 4].split(ConstantsValue.MUROS)[1]
										.split(ConstantsValue.TECHOS)[0].toUpperCase().replace("###", "").trim()));
					}
					if (tlineas > 6 && contenidoUb.toString().split("\n")[i + 5].contains(ConstantsValue.TECHOS)) {
						ubicacion.setTechos(
								fn.material(contenidoUb.toString().split("\n")[i + 5].split(ConstantsValue.TECHOS)[1]
										.split(ConstantsValue.INCENDIOMYM)[1].toUpperCase().replace("###", "").trim()));
						ubicacion.setMuros(
								fn.material(contenidoUb.toString().split("\n")[i + 5].split(ConstantsValue.MUROS)[1]
										.split(ConstantsValue.TECHOS)[0].toUpperCase().replace("###", "").trim()));
					}
					if (tlineas > 6
							&& contenidoUb.toString().split("\n")[i + 5].contains(ConstantsValue.NUMPISOSINCENDIO)) {
						ubicacion.setNiveles(Integer.parseInt(
								contenidoUb.toString().split("\n")[i + 5].split(ConstantsValue.INCENDIOMYMPT)[1]
										.replace("###", "").replace("\r", "").trim()));
					}
					...
					if (tlineas > 8
						...
						ubicacion.setNiveles(Integer.parseInt(
								contenidoUb.toString().split("\n")[i + 6].split(ConstantsValue.INCENDIOMYMPT)[1]
										.replace("###", "").replace("\r", "").trim()));
					}
			...
			if (contenidoUb.toString().isEmpty() && inicio > -1 && fin > inicio) {
				...
				for (int i = 0; i < newcontenido.split("\n").length; i++) {
					if (newcontenido.split("\n")[i]
							.contains(fn.palabraRgx(newcontenido.split("\n")[i], ConstantsValue.DIRECCION2))) {
						ubicacion.setCalle(newcontenido.split("\n")[i]
								.split(fn.palabraRgx(newcontenido.split("\n")[i], ConstantsValue.DIRECCION2))[1]
								.replace("###", "")
								.replace("\r", "").trim());
						if ((i + 2) < newcontenido.split("\n").length
								&& !newcontenido.split("\n")[i + 1].contains("###")
								&& fn.numTx(newcontenido.split("\n")[i + 1].trim()).length() > 0) {
					...
					if (newcontenido.split("\n")[i].contains("Origen:")
							&& newcontenido.split("\n")[i].contains("Destino")) {
						ubicacion.setCalle(newcontenido.split("\n")[i].split("Origen:")[1].split("Destino")[0]
								.replace("###", "").trim());
					}
					...
					if (newcontenido.split("\n")[i].contains(ConstantsValue.TECHOS)) {
						ubicacion.setTechos(fn.material(
								newcontenido.split("\n")[i].split(ConstantsValue.TECHOS)[1].split("Incendio:")[1]
										.toUpperCase().replace("###", "").trim()));
						ubicacion.setMuros(fn.material(newcontenido.split("\n")[i].split(ConstantsValue.MUROS)[1]
								.split(ConstantsValue.TECHOS)[0].toUpperCase().replace("###", "").trim()));
					}

					if (newcontenido.split("\n")[i].contains(ConstantsValue.NIVELES)) {
						if (newcontenido.split("\n")[i].split(ConstantsValue.NIVELES)[1]
								.contains(fn.palabraRgx(newcontenido.split("\n")[i], ConstantsValue.NUMSOTANOS))) {
							ubicacion.setNiveles(
									Integer.parseInt(newcontenido.split("\n")[i].split(ConstantsValue.NIVELES)[1].split(
											fn.palabraRgx(newcontenido.split("\n")[i], ConstantsValue.NUMSOTANOS))[0]
											.replace("###", "").replace("\r", "").trim()));
						} else {
							ubicacion
									.setNiveles(Integer
											.parseInt(newcontenido.split("\n")[i].split(ConstantsValue.NIVELES)[1]
													.split(fn.palabraRgx(
															newcontenido.split("\n")[i]
																	.split(ConstantsValue.NIVELES)[1],
															ConstantsValue.ENQUPISO))[0]
													.replace("###", "").replace("\r", "").trim()));
						}
					...
					if (newcontenido.split("\n")[i].contains(ConstantsValue.NUMPISOSINCENDIO)) {
						ubicacion.setNiveles(
								Integer.parseInt(newcontenido.split("\n")[i].split(ConstantsValue.INCENDIOMYMPT)[1]
										.replace("###", "").replace("\r", "").trim()));
					}
					if (newcontenido.split("\n")[i].contains("Giro:")) {
			...
			if (inicio > -1 && fin > -1 && inicio < fin) {
				...
				for (String x : arrNewContenido) {
					if (!x.contains("Tipo Vivienda") && !x.contains("Coberturas###Suma") && !x.contains("página#")
							&& !x.contains("No. Sótanos") && !x.contains("Tipo Techo")
				...
				if (resultado.toString().split("\r\n").length > 1) {
					...
					for (int i = 0; i < arrResultado.length; i++) {
						...
						if (a.contains(ConstantsValue.SECCION)) {
							...
							if ((a.split("###").length == 3 || a.split("###").length == 4)
									&& fn.castDouble(a.split("###")[0]) == null) {
								...
								if (deducible.toString().contains("de la pérdida")
									...
									if ((b.split("###").length == 1 || b.split("###").length == 2)
											&& !b.contains(ConstantsValue.SECCION) && nombresCoberturas
													.indexOf(b.split("###")[0].trim().toUpperCase()) == -1) {
										if (b.split("###").length == 2) {
									...
									if (c.split("###").length == 1
											&& (b.split("###").length == 1 || b.split("###").length == 2)
											&& !c.contains(ConstantsValue.SECCION)
											&& !coberturasNombreIncompleto.toString().contains(c)) {
								...
								coberturas.add(cobertura);
									...
									&& fn.numTx(arrResultado[i + 1].split("###")[0]).length() > 1
									&& a.trim().endsWith("Sublímite")
								...
								if (arrResultado[i + 1].split("###").length > 1) {
									cobertura.setDeducible(arrResultado[i + 1].split("###")[1].contains("%")
											? arrResultado[i + 1].split("###")[1].trim()
											: "");
								}
								coberturas.add(cobertura);
								deducible = new StringBuilder();
			...
			if (!modelo.getVigenciaDe().isEmpty()
					&& fn.diferenciaDias(modelo.getVigenciaDe(), modelo.getVigenciaA()) < 30) {
	...
	private String completaTextoCoberturas(String[] arrTexto, int i, StringBuilder coberturasNombreIncompleto) {
		...
		if (texto.contains("Según condición Fenómenos") && !texto.contains("Hidrometeorológicos")) {
			...
			switch (texto.split("###")[0]) {
				case "sublimite":
					texto = completaTextoActualConLineaSuperior(arrTexto, i, coberturasNombreIncompleto, "sublimite",
							"Robo de bienes y Valores de Empleados y Clientes");
					break;
				case "CONTEN":
					texto = completaTextoActualConLineaSuperior(arrTexto, i, coberturasNombreIncompleto, "CONTEN",
							"COBERTURA AMPLIA DE INCENDIO PARA");
					break;
				case "SUB":
					texto = completaTextoActualConLineaSuperior(arrTexto, i, coberturasNombreIncompleto, "SUB",
							"REMOCION DE ESCOMBROS CONTENIDOS");
					break;
				case "MOVIL DENTRO Y FUERA DEL PREDIO":
					texto = completaTextoActualConLineaSuperior(arrTexto, i, coberturasNombreIncompleto,
							"MOVIL DENTRO Y FUERA DEL PREDIO", "ROBO CON VIOLENCIA Y-O ASALTO EQUIPO");
					break;
				case FIJO_DENTRO_DEL_PREDIO:
					if (texto.split("###").length == 4 && arrTexto[i - 1].contains(ROBO_Y_O_ASALTO)
				...
				case ROBO_Y_O_ASALTO:
					if (i + 1 < arrTexto.length && arrTexto[i + 1].contains("###")
							&& arrTexto[i + 1].split("###")[0].equals(FIJO_DENTRO_DEL_PREDIO)
							&& arrTexto[i + 1].split("###").length == 3 && !texto.contains(FIJO_DENTRO_DEL_PREDIO)) {

						texto = texto.replace(ROBO_Y_O_ASALTO,
								"ROBO CON VIOLENCIA Y-O ASALTO DE EQUIPO FIJO DENTRO DEL PREDIO");
				...
				case "R.C. ARRENDATARIO SUB", "R.C. TALLERES CRISTALES":
					// linea actual nombre,suma linea siguiente complemento de suma asegurada,
					// deducible,coaseguro
					if (i + 1 < arrTexto.length && texto.split("###").length == 2) {
						texto = texto.replace("\r", "").replace("\n", "");
						if ((texto.split("###")[1].equals("Sublimite de")
								|| fn.numTx(texto.split("###")[1]).length() == 0) && arrTexto[i + 1].contains("###")
					...
					break;
				case "guerra":
					texto = completaTextoActualConLineaSuperior(arrTexto, i, coberturasNombreIncompleto, "guerra",
							"Apresamiento, secuestro o decomiso y actos de");
					break;
				case "labour":
					texto = completaTextoActualConLineaSuperior(arrTexto, i, coberturasNombreIncompleto, "labour",
							"Gastos de salvamento, remolque o auxilio, sue &");
					break;
				default:
					break;
			}
	...
	private String completaTextoActualConLineaSuperior(String[] arrTexto, int i,
			StringBuilder coberturasNombreIncompleto, String textoActual, String textSuperiorAcompletar) {
		String texto = arrTexto[i];
