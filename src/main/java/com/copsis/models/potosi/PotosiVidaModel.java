...
public class PotosiVidaModel {
	...
	public EstructuraJsonModel procesar(String contenido) {
		...
		try {
			...
			fin = fin == -1 ? contenido.indexOf("ASEGURADO SOLICITANTE") : fin;
			...
			for (int i = 0; i < newcontenido.toString().split("\n").length; i++) {
				...
				if (newcontenido.toString().split("\n")[i].contains(ConstantsValue.POLIZA_ACENT)
					...
					if (newcontenido.toString().split("\n")[i].split(ConstantsValue.POLIZA_ACENT)[1].split("###")[0]
							.length() > 2) {
						modelo.setPoliza(newcontenido.toString().split("\n")[i].split(ConstantsValue.POLIZA_ACENT)[1]
								.split("###")[0]);
					} else {
						modelo.setPoliza(newcontenido.toString().split("\n")[i].split(ConstantsValue.POLIZA_ACENT)[1]
								.split("###")[1]);
				...
				if (newcontenido.toString().split("\n")[i].contains("Razón Social")
						&& newcontenido.toString().split("\n")[i].contains("RFC")) {
					modelo.setCteNombre(newcontenido.toString().split("\n")[i].split("Social")[1].split("RFC")[0]
							.replace("###", ""));
					modelo.setRfc(
							newcontenido.toString().split("\n")[i].split("RFC")[1].replace("###", "").replace("-", ""));
				}
				if (newcontenido.toString().split("\n")[i].contains("Domicilio del Contratante")
						&& newcontenido.toString().split("\n")[i].contains("Tels")
						&& newcontenido.toString().split("\n")[i].contains("CP")) {
					modelo.setCteDireccion(
							newcontenido.toString().split("\n")[i].split("Contratante")[1].split("Tels")[0]
									.replace("###", ""));
			...
			for (int i = 0; i < newcontenido.toString().split("\n").length; i++) {
				...
				if (!newcontenido.toString().split("\n")[i].contains("Ramo")
					...
					beneficiario.setPorcentaje(fn
							.castDouble(newcontenido.toString().split("\n")[i].split("###")[2].trim().replace("%", ""))
							.intValue());
			...
			newcontenido = new StringBuilder();
			newcontenido.append(fn.extracted(inicio, fin, contenido)
					.replace("PAGO DE LA SUMA ASEGURADA POR INVALIDEZ",
							"PAGO DE LA SUMA ASEGURADA POR INVALIDEZ TOTAL Y PERMANENTE")
					.replace("EXENCIÓN DE PAGO DE PRIMAS POR",
			...
			modelo.setError(
					PotosiVidaModel.this.getClass().getTypeName() + " | " + ex.getMessage() + " | " + ex.getCause());
			return modelo;
		}
	}
