            ...
            for (int i = 0; i < lines.length; i++) {
                if (lines[i].contains("Póliza")) {
                    ...
                    if (matcher.find()) {
                        ...
                        if (i + 2 < lines.length && lines[i + 2].split("###").length > 0) {
                            modelo.setIdCliente(lines[i + 2].split("###")[lines[i + 2].split("###").length - 1]);
                        }

                        if (i + 1 < lines.length && lines[i + 1].split("###").length > 0) {
                            modelo.setMoneda(fn.moneda(lines[i + 1].split("###")[lines[i + 3].split("###").length - 1]));
                        }
